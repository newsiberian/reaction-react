...
// */
//
//export function reactionApps(optionHash) {
//  let fields;
//  let filter;
//  let key;
//  let match;
...
//                const registryLabel = registry.label ?
//                  registry.label.toCamelCase() : "";
...
import { translateRegistry } from "./i18n";
...
export default function getReactionApps(optionHash) {
  let fields;
  let filter;
  let key;
  let match;
          ...
          for (key in registryFilter) {
            ...
            if ({}.hasOwnProperty.call(registryFilter, key)) {
              ...
              if (match === Object.keys(registryFilter).length) {
                ...
                if (registry.enabled !== false) {
                  registry = translateRegistry(registry, app);
                  registry.enabled = registry.enabled || app.enabled;
                  registry.packageId = app._id;
